...
def execute_run(claimed: Claimed, *, store: RunStore, db: PitLaneDb, providers: dict,
    ...
    driver_name = thread["student_id"]  # using student_id for driver name
    system = SUPERVISOR_SYSTEM.format(driver_name=driver_name)
    tools = SupervisorTools(db, providers, driver_name, on_step=on_step)
    ...
    while True:
        for step_seq, call in pending:
            ...
            result, replayed = run_tool(tools, db, key, call["name"], call["args"])
            ...
            if on_step:
                on_step({"agent": "supervisor", "kind": "tool", "tool": call["name"], "args": call["args"],
                         "result": result, "ok": ok, "replayed": replayed, "ms": ms})
        pending = []

        if seq >= MAX_STEPS:
            raise AgentError("max_steps", "Too many steps. Try a simpler request.", False)
